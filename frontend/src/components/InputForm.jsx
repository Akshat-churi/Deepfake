...
import SynthesisDisplay from './SynthesisDisplay'; // Added import for synthesis component
                    ...
                    {result && !loading && (
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--glass-border)' }}>
                                ...
                                >
                                    CITIZEN REVIEWS
                                    {result.requiresCitizenReview && (<span style={{ background: 'var(--danger)', width: '8px', height: '8px', borderRadius: '50%', display: 'inline-block' }} title="Review Required"></span>)}
                                </button>
                                <button 
                                    className="tech-font"
                                    onClick={() => setActiveTab('synthesis')}
                                    style={{ padding: '0.5rem 1rem', background: 'none', border: 'none', borderBottom: activeTab === 'synthesis' ? '2px solid var(--accent-primary)' : '2px solid transparent', color: activeTab === 'synthesis' ? 'var(--text-primary)' : 'var(--text-secondary)', cursor: 'pointer', fontWeight: 'bold' }}
                                >
                                    SYNTHESIS
                                </button>
                            </div>
                            ...
                            {activeTab === 'reviews' && <VerificationFeed analysisId={result.id} requiresReview={result.requiresCitizenReview} result={result} />}
                            {activeTab === 'synthesis' && <SynthesisDisplay analysisId={result.id} result={result} />}
                        </div>
                    )}
                </div>
            </div>
